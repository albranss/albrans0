...
import random
APP_ID =27114545
APP_HASH='4d697dc98a6c882527aab3f72b25479d'
...
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('dev_albrans'))
	except BaseException :
		pass
...
async def join_channel(event):
	try:
		await abbas(JoinChannelRequest('@albrans0'))
	except BaseException :
		pass
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await shahm1.send_message(event.chat_id, f"تم الانتهاء من التجميع | Ab")
                break
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            try:
                ...
                chs += 1
                await event.edit(f"القناة رقم {chs}")
        await abbas.send_message(event.chat_id, "تم الانتهاء من التجميع | Al")
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع | Al")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await Alahm1.send_message(event.chat_id, f"تم الانتهاء من التجميع | Al")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await shahm1.send_message(event.chat_id, f"تم الانتهاء من التجميع | Al")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
...
async def OwnerStart(event): 
    ...
    msg = await abbas.get_messages(bot_username, limit=1)
    await msg[0].forward_to(onerabbas_id)
@abbas.on(events.NewMessage(outgoing=False, pattern=r'.معلومات دعمكم'))
async def OwnerStart(event): 
    sender = await event.get_sender()
    if sender.id == onerabbas_id:
        send = await abbas.send_message(bot_username8, '/start')
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinc = await 
  abbas(JoinChannelRequest('dev_albrans'))
